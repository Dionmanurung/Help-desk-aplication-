    ...
    public function role2()
{
    ...
    public function suspend($duration)
    {
        $this->is_suspended = true;
        $this->suspend_start_time = now();
        $this->suspend_duration = $duration;
        $this->save();
    }
    ...
    public function unsuspend()
    {
        ...
        $this->suspend_start_time = null;
        $this->suspend_duration = null;
        $this->save();
    }

    public function isSuspended()
    {
        if ($this->is_suspended && $this->suspend_start_time && $this->suspend_duration) {
            $suspendEndTime = $this->suspend_start_time->addMinutes($this->suspend_duration);
            return $suspendEndTime->isFuture();
        }
